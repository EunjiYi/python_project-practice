...
for tc in range(1, int(input()) + 1):
    ...
    # 맨 뒤에서부터 조회해서 처음으로 0이 아닌 줄을 가져온다.
    ...
    #         if matrix[i][j] != '0':
    #             temp = matrix[i][:]
    #             if temp not in not_zero:
    #                 not_zero.append(temp)
    #print(not_zero) #조사해야할 모든 줄

    # 0이 아닌 줄만 2진수로 전부 변환한다.
    binary_not_zero = [''] * n
    for i in range(len(matrix)):
        #tmp1 = []
        for j in range(m):
            #tmp1 += hex_to_binary(not_zero[i][j]) # 시간초과나서 하드코딩함.(잘하는 건가...)
            binary_not_zero[i] += dic3[matrix[i][j]]
        #inary_not_zero.append(tmp1)
    #print(len(binary_not_zero)) #조사해야할 모든 줄을 2진수로 바꾼 것.
